...
async function autoCheckAndResolve(env){
  ...
  for(const g of groups){
    ...
    await env.KV.put('ips:'+g.id,JSON.stringify(gips));
    ...
    gr.push({id:g.id,name:g.name,domain:g.domain,ok,err,count:gips.length,resolved:resolved.map(i=>i.ipPort+'('+i.checkLatency+'ms)')});
  }
  // 统计失效原因
...
<button class="btn p" onclick="doChk(this)">🔍 检测+解析全部分组</button>
</div><p style="color:var(--dm);font-size:11px;margin-top:6px">并发${BATCH}，超时10秒，重试${RETRY}次，总时限4.5分钟</p></div>
...
const Q="'";
const tabs=[['ov','概览'],['ip','IP管理'],['gr','分组管理'],['bl','黑名单'],['st','设置']];
$('nav').innerHTML=tabs.map(([k,v],i)=>'<a onclick="sw('+Q+k+Q+')" id="n-'+k+'"'+(i===0?' class="on"':'')+'>'+v+'</a>').join('');
function sw(k){document.querySelectorAll('.tab').forEach(t=>t.classList.remove('on'));document.querySelectorAll('nav a').forEach(a=>a.classList.remove('on'));$('t-'+k)?.classList.add('on');$('n-'+k)?.classList.add('on');if(k==='ov')loadSt();if(k==='ip'&&CG)chgGrp();if(k==='gr')loadGrps();if(k==='bl')loadBL();if(k==='st')loadCfg()}
...
// 设置
async function loadCfg(){try{const c=await api('/api/config');$('c-tt').value=c.tgToken||'';$('c-tc').value=c.tgChatId||''}catch{}}
async function saveCfg(){
  const c={tgToken:$('c-tt').value,tgChatId:$('c-tc').value};
